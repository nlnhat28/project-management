        ...
        public async Task<IPagedResult<bool>> DeleteAsync(long id, string username)
        {
            using (var tran = await _unitOfWork.OpenTransaction())
            {
                try
                {
                    ...
                    var delete = await _unitOfWork.StudentRepository.GetSingleAsync(x => x.Id == id && !x.IsDeleted);
        ...
        public async Task<IPagedResult<bool>> SaveAsync(Student request)
        {
            using (var tran = await _unitOfWork.OpenTransaction())
            {
                try
                {
                    ...
                    {
                        var checkDuplicate = await _unitOfWork.StudentRepository.GetSingleAsync(x => x.Username == request.Username && !x.IsDeleted);
                        if (checkDuplicate != null)
                        {
                            return new PagedResult<bool>
                            {
