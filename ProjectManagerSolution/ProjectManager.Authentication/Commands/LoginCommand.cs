...
namespace ProjectManager.Authentication.Commands
{
    ...
    public class LoginCommand : ILoginCommand
    {
        ...
        public async Task<Response<AuthenticateResponse>> Authenticate(LoginRequest request)
        {
            ...
            {
                var user = await _unitOfWork.StudentRepository.GetSingleAsync(x => x.Username.ToLower() == request.UserName.ToLower() && x.Password == EncriptFunctionsHelper.GeneratePassword(request.Password) && !x.IsDeleted);
                if (user == null)
                {
            ...
            {
                var user = await _unitOfWork.TeacherRepository.GetSingleAsync(x => x.Username.ToLower() == request.UserName.ToLower() && x.Password == EncriptFunctionsHelper.GeneratePassword(request.Password) && !x.IsDeleted);
                if (user == null)
                {
                    return new Response<AuthenticateResponse>
                    {
            ...
            {
                var user = await _unitOfWork.UserManagementRepository.GetSingleAsync(x => x.Username.ToLower() == request.UserName.ToLower() && x.Password == EncriptFunctionsHelper.GeneratePassword(request.Password) && !x.IsDeleted);
                if (user == null)
                {
                    return new Response<AuthenticateResponse>
                    {
